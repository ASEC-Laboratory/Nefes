...
- **Fluctuations** carry a prime, $X' = X - \overline{X}$, and are the small unsteady departures from the mean. They are the *organized* (coherent, phase-resolved) fluctuations that the acoustic layer resolves; where the *stochastic* turbulent departure must be named alongside them it carries a double prime, $X''$, and it is closed by the constitutive models rather than resolved (see [governing equations](theory/governing-equations.md#sec-govern-turbulence)).
...
- **Section (area) averages** carry angle brackets, $\langle X\rangle \equiv \tfrac{1}{A}\int_A X\,\mathrm{d}A$, and denote the average of a field over a port cross-section; the edge quantities are averages of this kind (see [network abstraction](theory/abstraction.md#sec-abstraction-averages)).
