...
The equilibrium solve uses an element-potential (CEA-style) Gibbs minimization [@gordon_mcbride_1994], formulated to be branch-free so that the complex-step derivative propagates through it natively (see [complex-step](../design/complex-step.qmd)).
...
The expansion is linear, so the library only ever receives an elemental (or species) composition and never a network-specific mixture fraction — the separation that keeps the library independent of the network.
