...
The stability eigensolver, however, searches the complex plane for the roots of $\det\mathbf{A}(\omega)$ and therefore needs an analytic function, so the table is continued by one of the two fits of [dynamic sources](dynamic-sources.qmd) — the impulse-response fit when the recovered response dies out in finite time (a flame, a compact element), which is the recommended route, or the barycentric-rational (AAA) fit [@nakatsukasa_2018] when it carries a genuine resonance; by default the identification applies the rational fit, since it needs no memory-length input.
The same continued object then drives both the real-axis Nyquist analysis and the complex-plane eigensolver, and the conversions between transfer and scattering forms preserve its analyticity (test: `test_identify_transfer_matrix_continuation_is_analytic`).
