...
## Parrott helicopter exhaust muffler {#sec-bench-parrott-muffler}

Where the canonical two-ports check single elements, this case checks a whole reactive silencer assembled from geometry: the three-stage exhaust muffler that [@parrott_1973] designed and flight-tested for a helicopter, the same device reproduced in Fig 5.27 of [@dokumaci_2021].
Its Fig 13 tabulates twelve components — three concentric extended-tube expansion chambers in series — with published lengths and areas, running an exhaust at a sound speed of $610\;\mathrm{m/s}$ and a tailpipe Mach number of $0.1$.

Nefes assembles the muffler from that geometry: each extended tube becomes an area change carrying the through flow and a rigid annular side branch closed by the chamber end wall, and the mean flow is solved before the acoustics.
The reference is the classical transfer-matrix (four-pole) method in the $(p, U)$ variables, an independent computation that shares no code with the network solver.
Across $20$–$1500\;\mathrm{Hz}$ the two transmission-loss spectra agree to a root-mean-square of about $10^{-5}\;\mathrm{dB}$, the only larger residual sitting on a stub resonance above $80\;\mathrm{dB}$ where the reference's $\cot(kl)$ is singular (test: `test_muffler_matches_transfer_matrix`); the single-chamber limit matches the closed-form transmission loss to machine precision (test: `test_plain_chamber_matches_analytic`).

The reading of the geometry is checked a second way, against Parrott's own published spectrum rather than a re-implementation of his method: his Fig 9 curve, digitised for this muffler, has the same peak and null frequencies as the Nefes spectrum, though its peaks are rounded near $45\;\mathrm{dB}$ where the lossless model is razor-sharp.
That rounding is dissipation at the branch end walls, which Parrott modelled with a below-unity reflection factor; giving the stub walls an absorptive reflection $R<1$ and freezing them — so the response folds each wall in at its declared closure and the two-port reads out exactly as in the rigid case — carries the lossless response down onto Parrott's curve and the measured points, filling the deep nulls and rounding the peaks.
It matches an independent lossy transfer-matrix to machine precision at $R=0.8$, so the loss it adds is physical rather than numerical (test: `test_absorptive_stub_walls_match_lossy_transfer_matrix`).

Solving the mean flow at the design mass flow recovers the operating point the reference assumes — Mach $0.1$ in the pipes, an order of magnitude less in the wide chambers, and zero in the closed stubs (test: `test_mean_flow_operating_point`) — and confirms that the mean flow barely moves the anechoic transmission loss, shifting the sharp resonances by about $0.1\,\%$ and leaving the smooth band within a few tenths of a decibel (test: `test_mean_flow_leaves_anechoic_tl_nearly_unchanged`), which is Parrott's own conclusion that the internal flow corrections are insignificant.
The muffler's measured field transmission losses (its Table II) are carried in the notebook as real-world context: as in the reference's own Fig 17, the reactive-muffler prediction captures where the attenuation lives but sits above the measured lower bounds, which are contaminated by the finite source impedance and by muffler self-noise.
The example notebook `examples/validation/parrott_helicopter_muffler.ipynb` walks the case through.

Taken together, the benchmarks confirm that the framework reproduces a steady compressible network, a self-excited thermoacoustic instability with its analytic frequencies and growth rates, the indirect-noise chain with its correct negative control, a real reactive muffler against the transfer-matrix method, and the canonical acoustic two-ports.
