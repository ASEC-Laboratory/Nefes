...
def _contour_to_fg(c, n=181):
    ...
    ``(Re(omega)/2*pi, -Im(omega))``.  A :class:`~nefes.perturbation.stability.contour.Contour` (with
...
def plot_spectrum(
    ...
    contour : Contour or sequence, optional
        ...
        :class:`~nefes.perturbation.stability.contour.Contour`, a list of them, or an array of complex
...
def plot_mode_shape(shape, *, labels=None, positions=None, title="Mode shape", **layout):
    ...
    **layout
    ...
    fig.update_layout(template=NEFES_TEMPLATE_NAME, title=title)
    fig.update_layout(**layout)
    return fig
