"""Round-1 acoustic network: transfer / scattering matrices (theory.md s12.7 (i)).
...
with ``M = 0`` and ``S`` a no-op provision.  Targets are closed-form duct
acoustics plus internal consistency (cascade composition, unitarity).
"""
...
from fns.derive import ES_C, ES_U
...
def test_quiescent_duct_transmission_phase():
    ...
    resp = acoustic_response(prob, res.x, OM)
    ...
    # cross-check the standalone single-duct phase helper
...
def test_meanflow_duct_tau_plus_phase():
    ...
    # with mean flow this is the sharp test: wrong signs give tau_- instead
    assert np.allclose(trans, np.exp(-1j * OM * tau_p), atol=1e-9)
...
def test_reextraction_without_resolve():
    ...
    resp = acoustic_response(prob, res.x, OM)
    # different edge pairs from the SAME stored response
    T01 = resp.transfer_matrix(0, 1)
    T03 = resp.transfer_matrix(0, 3)
    S13 = resp.scattering_matrix(1, 3)
    assert T01.shape == (OM.size, 2, 2)
    assert T03.shape == (OM.size, 2, 2)
    assert S13.shape == (OM.size, 2, 2)
    # first duct sub-matrix matches its analytic phase
    est = states_table(prob, res.x)
    c, u = est[ES_C, 0], est[ES_U, 0]
    diag = np.array([np.diag([np.exp(-1j * w * 0.7 / (u + c)), np.exp(1j * w * 0.7 / (c - u))]) for w in OM])
    assert np.allclose(T01, diag, atol=1e-9)


# -- 4. lossless unitarity --------------------------------------------------


def test_quiescent_scattering_unitary():
    prob, res = _single_duct(101325.0, 101325.0, 1.0)
    resp = acoustic_response(prob, res.x, OM)
    S = resp.scattering_matrix(0, 1)
    for i in range(OM.size):
        assert np.allclose(S[i].conj().T @ S[i], np.eye(2), atol=1e-9)
        assert abs(abs(np.linalg.det(S[i])) - 1.0) < 1e-9
...
def test_terminal_detection():
    prob, _ = _single_duct(110000.0, 101325.0, 1.0)
...
def test_verifier_rejects_supersonic_duct():
    # choke the duct hard: huge pressure ratio -> sonic throat, supersonic edge
    prob, res = _single_duct(500000.0, 50000.0, 1.0, area=0.05)
    est = states_table(prob, res.x)
...
def test_zero_frequency_duct_is_continuity():
    prob, res = _single_duct(101325.0, 101325.0, 1.0)
    resp = acoustic_response(prob, res.x, np.array([0.0]))
    T = resp.transfer_matrix(0, 1)[0]
    assert np.allclose(T, np.eye(2), atol=1e-6)  # DC limit: lossless pass-through
...
def test_flame_face_is_wired_but_unimplemented():
    # manually tag an element with the reserved flame face: the dispatch hook
    # must fire (NotImplementedError) at assembly, proving S(omega) is wired.
    net = [cat.total_pressure_inlet(110000.0, 300.0), cat.duct(1.0), cat.pressure_outlet(101325.0, 300.0)]
...
def test_cascade_quiescent_unitary():
    ...
    # acoustic power ~ rho*c*A*(|f|^2 - |g|^2).  The *power-normalized* scattering
    # matrix (waves scaled by sqrt(rho*c*A)) is unitary for a lossless quiescent
    # cascade.
    from fns.derive import ES_RHO, ES_AREA
    ...
    resp = acoustic_response(prob, res.x, OM)
    S = resp.scattering_matrix(0, 3)
    # exact unitarity holds only at u = 0; the smoothed "quiescent" solve leaves a
    # residual mean Mach ~1e-6, so the leak scales to ~1e-6 (not machine zero).
    for i in range(OM.size):
