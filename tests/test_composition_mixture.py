...
def lib():
    return SpeciesSet.from_cea(species=["CH4", "O2", "N2", "CO2", "H2O", "H2"])
...
def test_stoichiometric_methane_air_is_9_5_percent(lib):
    mix = equivalence_ratio_mixture({"CH4": 1.0}, AIR, 1.0, species_set=lib)
...
def test_rich_is_positive_lean_is_negative_balance(lib):
    rich = equivalence_ratio_mixture({"CH4": 1.0}, AIR, 2.0, species_set=lib)
    lean = equivalence_ratio_mixture({"CH4": 1.0}, AIR, 0.5, species_set=lib)
...
def test_phi_scales_fuel_oxidizer_mole_ratio(lib):
    ...
    one = equivalence_ratio_mixture({"CH4": 1.0}, AIR, 1.0, species_set=lib)
    two = equivalence_ratio_mixture({"CH4": 1.0}, AIR, 2.0, species_set=lib)
    r1 = one["CH4"] / one["N2"]
    r2 = two["CH4"] / two["N2"]
...
def test_hydrogen_oxygen_stoichiometric_is_two_to_one(lib):
    mix = equivalence_ratio_mixture({"H2": 1.0}, {"O2": 1.0}, 1.0, species_set=lib)
...
def test_phi_zero_returns_pure_oxidizer(lib):
    mix = equivalence_ratio_mixture({"CH4": 1.0}, AIR, 0.0, species_set=lib)
...
def test_mass_basis_sums_to_one_and_differs_from_mole(lib):
    mole = equivalence_ratio_mixture({"CH4": 1.0}, AIR, 1.0, basis="mole", species_set=lib)
    mass = equivalence_ratio_mixture({"CH4": 1.0}, AIR, 1.0, basis="mass", species_set=lib)
...
def test_mass_basis_fuel_input(lib):
    ...
    by_mole = equivalence_ratio_mixture({"CH4": 1.0}, AIR, 1.0, species_set=lib)
    by_mass = equivalence_ratio_mixture({"CH4": 1.0}, AIR, 1.0, fuel_basis="mass", species_set=lib)
    assert by_mass["CH4"] == pytest.approx(by_mole["CH4"], rel=1e-12)


def test_rejects_bad_arguments(lib):
    with pytest.raises(ValueError, match="non-negative"):
        equivalence_ratio_mixture({"CH4": 1.0}, AIR, -1.0, species_set=lib)
    with pytest.raises(ValueError, match="no net oxygen demand"):
        equivalence_ratio_mixture({"N2": 1.0}, AIR, 1.0, species_set=lib)  # N2 is not a fuel
    with pytest.raises(ValueError, match="supplies no oxygen"):
        equivalence_ratio_mixture({"CH4": 1.0}, {"N2": 1.0}, 1.0, species_set=lib)  # N2 is not an oxidizer
