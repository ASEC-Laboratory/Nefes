...
`fns` mean-flow network solver. Per the design decision below, the thermochemistry is
developed as a **separate, standalone library** with no dependency on `fns`; the
...
- **Part B** — the `fns` integration that consumes it.
...
- **AD-1.** The thermochemistry/equilibrium/kinetics functionality **shall** be developed
  ...
  release cycle, **not tied to `fns`**. During development it is consumed via an
...
- **AD-2.** **Dependency direction is one-way:** `fns` depends on the library; the
  library **shall not** import or assume anything from `fns` (no notion of edges,
...
- **AD-3.** **Boundary of responsibility:**
  ...
  - **`fns`** owns: the mapping from edge unknowns `(mdot, p, h_t)` to a thermodynamic
...
> **Governing constraint (G-1).** `fns` builds its Jacobian by **complex-step
> differentiation** ([`solver.complex_step_jacobian`](fns/solver.py)), which requires
...
> friendliness), not as a `fns`-specific coupling.
...
- **D-1.** **Transport absolute total enthalpy** (including formation enthalpy) in
  `fns`. HP equilibrium conserves enthalpy, so adiabatic constant-pressure reaction
...
- **D-2.** **Transport conserved scalars, slave species to equilibrium.** `fns`
...
- **D-4.** In `fns`, **generalize the single hard-wired `h_t` advection into a scalar
  registry**; `h_t` becomes the first registered scalar; the system stays square.
- **D-5.** **Reaction is localized to elements** in `fns`, consistent with the existing
...
- **R-A1.1** The library **shall** have **no dependency** on `fns` or any network
  concept (AD-2).
...
- **R-A6.1** Each backend **shall** declare one of two differentiation modes:
  ...
  2. *sensitivity-providing* — returns values plus an analytic Jacobian block for a live
     ...
     in [`state.solve_density`](fns/state.py)).
- **R-A6.2** No in-library residual/property path **shall** use `abs`/`sign`/`max` or
  ...
  [`fns.smooth`](fns/smooth.py), but the library **shall** carry its own copy — no
  `fns` import).
...
- **R-A8.1 (standalone core).** The library **shall** follow the "standalone core solver"
  philosophy of `implementation-plan.md` §1 (the standalone, JIT-friendly core solver): minimal object coupling, and **should** be
...
# PART B — `fns` integration (consumer of `thermolib`)

These requirements live in `fns` and depend on Part A. They keep all network concepts
out of the library.

## B.1 Closure adapter
- **R-B1.1** `fns` **shall** define a **closure adapter** that maps edge unknowns to a
...
- **R-B1.2** The solver **shall** access all thermochemistry **only** through this adapter;
  [`state.recover_state`](fns/state.py) and the choking rows **shall not** reference a
...
- **R-B2.2** The fixed point between `h = h_t − u²/2`, `u = mdot/(ρA)`, and the library's
  ...
  pattern used today in [`state.solve_density`](fns/state.py).
...
- **R-B4.3** The default donor rule **shall** be the existing mass-weighted smooth-upwind
  mix ([`Element.donor_enthalpy`](fns/elements.py) generalized); a passive scalar
...
- **R-B4.6** Each scalar **shall** carry a variable/residual scale for the solver's
  nondimensionalization ([`network.variable_scales`/`residual_scales`](fns/network.py)).
- **R-B4.7 (composition descriptor abstraction).** `fns` **shall** provide one
...
- **R-B5.5 (continuation).** A **Damköhler / chemistry-source homotopy** **should** be
  ...
  ([`solver.solve`](fns/solver.py)), using the equilibrium solution as the start
